...
export namespace vk {
    inline namespace v6 {
         ...
         *
         * Used for mainly performing compile-time checks if the features
         * specified are actual valid features
         *
         * Because Vulkan is a C API and .pNext being a void*. This can be
         * bug-prone for accidentally pointing to a type that is not a feature
         * struct like a std::string, for example.
         *
         * VulkanChainable concept is used to verify the conditions of the type
         * that is being passed to chain_features.
         *
         * Which is important to ensure that we only accept types that do
         * contain .sType and .pNext
         *
         * TODO: Probably might have this be a template specialization due to
         * being able to verify at compile-time if the vulkan features, we need
         * to check for are valid. This way even if the struct has `.sType` and
         * `.pNext`, we can at least verify the struct is a feature that is
         * valid
         *
         *
         * Example:
         ...
         * reached.
         *      // Users should not be allowed to supply a non vulkan feature
         * struct ONLY the vulkan feature struct static_assert(*this, "Invalid
         * vulkan feature specified. Only accepting valid vulkan features.");
         * };
         */
        template<typename T>
        ...
        public:
            device_features(Features&&... p_args)
              : m_data(std::forward<Features>(p_args)...) {
                ...
                // Then we chain the pNext pointer altogether for each feature
                // we unpack if there are any
                if constexpr (sizeof...(Features) > 0) {
                    std::apply(
                      [this](auto&... spec) { chain_features(spec...); },
                      m_data);
                }
            }
             ...
             *
             * @returns the internal data of the vulkan device extensions that
             * are chained altogether.
             *
        ...
        private:
             ...
             * enable.
             *
             * This linked-list feature is used to enable modern GPU features.
             * This internal API allows to take in a collection of structs that
             * are inter-connected them into a single chain the internal API's
             * can read from.
             *
            ...
            void chain_features(T& head, Feature&... tail) {
                 ...
                 * Compiler does not generate a loop, rather it'll generate a
