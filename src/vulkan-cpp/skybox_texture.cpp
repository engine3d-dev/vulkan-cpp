...
namespace vk {
    ...
    skybox_texture::skybox_texture(const VkDevice& p_device,
                                   const skybox_texture_info& p_config)
      : m_device(p_device) {
        ...
        // bytes_per_texture_format(cubemap_config.format); uint32_t
        // layer_size_with_bytes = cubemap_config.extent.width *
        // cubemap_config.extent.height * bytes_per_pixel; uint32_t layer_count
        ...
        for (size_t i = 0; i < p_config.faces.size(); i++) {
            int channels;
            image_pixel_data[i] = stbi_load(p_config.faces[i].c_str(),
                                            &m_width,
                                            &m_height,
                                            &channels,
                                            STBI_rgb_alpha);
            ...
            if (!image_pixel_data[i]) {
                std::println("Cannot load face[{}] = filanme with {}",
                             i,
                             p_config.faces[i]);
                return;
            }
            ...
            int bytes_per_pixel =
              bytes_per_texture_format(skybox_image_info.format);
            uint64_t layer_size_with_bytes =
              (uint64_t)skybox_image_info.extent.width *
              skybox_image_info.extent.height * bytes_per_pixel;
            int layer_count = 1;
            uint64_t image_size = layer_size_with_bytes * layer_count;

            int property_flag = memory_property::host_visible_bit |
            ...
            buffer_handle staging_buffer =
              create_buffer(m_device, staging_buffer_config);
            write_info write_staging_buffer = { .offset = i * image_size,
                                                .size_bytes =
                                                  layer_size_with_bytes };

            write(
              m_device, staging_buffer, image_pixel_data, write_staging_buffer);
        ...
        //     .format = VK_FORMAT_R8G8B8A8_UNORM,
        //     .usage = (VkImageUsageFlagBits)VK_IMAGE_USAGE_TRANSFER_DST_BIT |
        //     VK_IMAGE_USAGE_SAMPLED_BIT, .image_flags =
        //     VK_IMAGE_CREATE_CUBE_COMPATIBLE_BIT, .array_layers = 6,
        //     .physical_device = p_config.physical_handle
        // };
        // m_skybox_image = vk::sample_image(m_device, skybox_image_info);

        // int bytes_per_pixel =
        // bytes_per_texture_format(skybox_image_info.format); int
        // layer_size_with_bytes =
        // static_cast<int>(skybox_image_info.extent.width *
        // skybox_image_info.extent.height * bytes_per_pixel);
        ...
        // int property_flag = memory_property::host_visible_bit |
        // memory_property::host_cached_bit; buffer_configuration
        ...
        // buffer_handle staging_buffer = create_buffer(m_device,
        // staging_buffer_config);

        // for(size_t i = 0; i < 6; i++) {
        //     std::println("i * image_size = {}", (i * image_size));
        //     write(m_device, staging_buffer, image_pixel_data, i *
        //     image_size);
        //     // std::println("Mapping i = {}", i);
        //     // void* mapped = nullptr;
        //     // vk_check(
        //     // vkMapMemory(
        //     //     p_device, staging_buffer.device_memory, i * image_size,
        //     image_size, 0, &mapped),
        //     // "vkMapMemory");
        ...
        // [per-face filename] => [staging buffer] => [image] ->
        // image_layout_transition => copy operation => transfer to color
        // attachment layout
