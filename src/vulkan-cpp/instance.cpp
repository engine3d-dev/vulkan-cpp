...
namespace vk {
    ...
    instance::instance(const application_configuration& p_config, const debug_message_utility& p_debug_message_utils) {
        ...
        for(const VkLayerProperties property : layer_properties) {
            m_layer_properties.emplace_back(property.layerName, property.specVersion, property.implementationVersion, property.description);
        }

        // Setting up instance extensions
        instance_ci.enabledExtensionCount = static_cast<uint32_t>(p_config.extensions.size());
        instance_ci.ppEnabledExtensionNames = p_config.extensions.data();

#if _DEBUG
        // Setting up validation layers
        instance_ci.enabledLayerCount = static_cast<uint32_t>(p_config.validations.size());
        ...
        VkDebugUtilsMessengerCreateInfoEXT debug_create_info = {
            ...
            .messageSeverity = to_debug_message_severity(p_debug_message_utils.severity),
...
#endif
        vk_check(vkCreateInstance(&instance_ci, nullptr, &m_instance), "vkCreateInstance");
    }
