...
namespace vk {
    ...
    buffer_handler::buffer_handler(const VkDevice& p_device, const buffer_settings& p_settings) : m_device(p_device) {
        ...
        // 3. selects the required memory requirements for this specific buffer allocations
        uint32_t memory_index = select_memory_requirements(p_settings.physical_memory_properties, memory_requirements, p_settings.property_flags);
        ...
        vk_check(vkAllocateMemory(p_device, &memory_alloc_info, nullptr, &m_device_memory),"vkAllocateMemory");

        // 5. bind memory resource of this buffer handle
        vk_check(vkBindBufferMemory(p_device, m_handle, m_device_memory, 0),"vkBindBufferMemory");
    }
