...
void
transition_image_layout(VkDevice p_device,
...
main() {
    ...
    while (!glfwWindowShouldClose(window)) {
        ...
        glm::mat4 view_only = glm::mat4(glm::mat3(ubo.view));
        glm::mat4 skybox_proj = glm::perspective(glm::radians(45.f),
                                         static_cast<float>(swapchain_extent.width) / static_cast<float>(swapchain_extent.height),
                                         0.1f, 10.0f);
        skybox_proj[1][1] *= -1;

        skybox_uniform skybox_ubo = {
            .proj_view = skybox_proj * view_only,
        };
        test_cubemap.update_uniform(skybox_ubo);
