...
public:
    cubemap(const VkDevice& p_device,
      ...
      : m_device(p_device) {
        ...
        // --- Shaders / descriptors / pipeline (match shader_samples/sandbox-shader-samples) ---
        const std::filesystem::path shader_dir =
          std::filesystem::current_path() / "shader_samples" /
          "sandbox-shader-samples";

        const std::array<vk::shader_source, 2> sources = {
            vk::shader_source{
              .filename = (shader_dir / "skybox.vert.spv").string(),
              .stage = vk::shader_stage::vertex,
            },
            vk::shader_source{
              .filename = (shader_dir / "skybox.frag.spv").string(),
        ...
        vk::shader_resource_info shader_info = {
            .sources = sources,
            .vertex_attributes = {}, // skybox uses gl_VertexIndex (no vertex buffers)
        };
        ...
        m_skybox_ubo.transfer(std::span<const skybox_uniform>(&identity, 1));
        ...
        vk::pipeline_params pipe_info = {
            ...
            .vertex_attributes = {},      // no vertex input
            .vertex_bind_attributes = {}, // no vertex input
    ...
    void draw(const VkCommandBuffer& p_current) {
        bind(p_current);
        vkCmdDraw(p_current, 36, 1, 0, 0);
    }

    void destroy() {
        // m_cubemap.destroy();
        // m_skybox_shaders.destroy();
        // m_skybox_destroy.destroy();
        // m_skybox_pipeline.destroy();
        // m_skybox_uniform.destroy();
